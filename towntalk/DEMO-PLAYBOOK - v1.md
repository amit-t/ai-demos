...
- [ ] Phone ready with `http://[your-ip]:3000/mobile.html` bookmarked
- [ ] QR code pre-printed on a slide (generate it from `http://[your-ip]:3000/mobile.html`)
- [ ] Venue WiFi tested with multiple devices
...
Verify you see the server banner. Open `http://localhost:3000` — it will 404 for display.html (because you deleted it). That's correct — you're about to build it live.
...
- The overlay fetches the server URL from `/api/info` (returns `{ "url": "http://192.168.x.x:3000" }`)
...
- Verify audience is on the same WiFi network
- Have the URL on a slide as backup: `http://[your-ip]:3000/mobile.html`
...
**QR Code Overlay:**
- Press Q to toggle. Fetches server URL from `/api/info` endpoint, generates QR code using qrcodejs from cdnjs pointing to `{url}/mobile.html`

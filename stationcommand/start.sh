#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────
# Station Command — Start server + public tunnel
#
# Tries (in order):  ngrok → cloudflared → localtunnel
# Once the tunnel URL is detected, it's pushed to the server
# so the QR code on the display page uses the public URL.
# ──────────────────────────────────────────────────────────

set -e

PORT=3002
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
TUNNEL_PID=""
SERVER_PID=""

cleanup() {
  echo ""
  echo "  Shutting down…"
  [ -n "$TUNNEL_PID" ] && kill "$TUNNEL_PID" 2>/dev/null
  [ -n "$SERVER_PID" ] && kill "$SERVER_PID" 2>/dev/null
  wait 2>/dev/null
  echo "  Done."
  exit 0
}
trap cleanup EXIT INT TERM

# ─── Kill anything hogging the port ──────────────────────────
EXISTING_PID=$(lsof -ti :"$PORT" 2>/dev/null || true)
if [ -n "$EXISTING_PID" ]; then
  echo "  🔪 Killing process(es) on port $PORT: $EXISTING_PID"
  kill -9 $EXISTING_PID 2>/dev/null || true
  sleep 1
fi

# ─── Start the Node server ───────────────────────────────────
echo ""
echo "  Starting Station Command server on port $PORT…"
cd "$SCRIPT_DIR"
node server.js &
SERVER_PID=$!
sleep 2

# Verify server is running
if ! curl -s "http://localhost:$PORT/api/info" > /dev/null 2>&1; then
  echo "  ❌ Server failed to start. Check for errors above."
  exit 1
fi
echo "  ✅ Server running on http://localhost:$PORT"
echo ""

# ─── Try tunnels in order ────────────────────────────────────

start_tunnel() {
  local TUNNEL_URL=""

  # ── Option 1: ngrok ──────────────────────────────────────
  if command -v ngrok &> /dev/null; then
    echo "  🔍 Found ngrok — starting tunnel…"
    ngrok http "$PORT" --log=stdout --log-level=info > /tmp/stationcommand-ngrok.log 2>&1 &
    TUNNEL_PID=$!

    # Try multiple detection methods for up to 15 seconds
    for i in $(seq 1 30); do
      sleep 0.5

      # Method 1: ngrok local API (works on most versions)
      if [ -z "$TUNNEL_URL" ]; then
        TUNNEL_URL=$(curl -s http://localhost:4040/api/tunnels 2>/dev/null \
          | grep -oE 'https://[a-zA-Z0-9._-]+\.ngrok[a-zA-Z0-9.-]*\.(app|io|dev)' \
          | head -1)
      fi

      # Method 2: parse the log file for the URL
      if [ -z "$TUNNEL_URL" ]; then
        TUNNEL_URL=$(grep -oE 'https://[a-zA-Z0-9._-]+\.ngrok[a-zA-Z0-9.-]*\.(app|io|dev)' /tmp/stationcommand-ngrok.log 2>/dev/null | head -1)
      fi

      if [ -n "$TUNNEL_URL" ]; then break; fi
    done

    if [ -n "$TUNNEL_URL" ]; then
      echo "  ✅ ngrok tunnel: $TUNNEL_URL"
      push_tunnel_url "$TUNNEL_URL"
      return 0
    else
      echo "  ⚠️  ngrok started but couldn't detect URL."
      echo "     Check: cat /tmp/stationcommand-ngrok.log"
      echo "     Or open: http://localhost:4040"
      kill "$TUNNEL_PID" 2>/dev/null
      TUNNEL_PID=""
    fi
  fi

  # ── Option 2: cloudflared ────────────────────────────────
  if command -v cloudflared &> /dev/null; then
    echo "  🔍 Found cloudflared — starting tunnel…"
    cloudflared tunnel --url "http://localhost:$PORT" > /tmp/stationcommand-cf.log 2>&1 &
    TUNNEL_PID=$!

    for i in $(seq 1 30); do
      sleep 0.5
      TUNNEL_URL=$(grep -o 'https://[a-z0-9-]*\.trycloudflare\.com' /tmp/stationcommand-cf.log 2>/dev/null | head -1)
      if [ -n "$TUNNEL_URL" ]; then break; fi
    done

    if [ -n "$TUNNEL_URL" ]; then
      echo "  ✅ cloudflared tunnel: $TUNNEL_URL"
      push_tunnel_url "$TUNNEL_URL"
      return 0
    else
      echo "  ⚠️  cloudflared started but couldn't detect URL."
      kill "$TUNNEL_PID" 2>/dev/null
      TUNNEL_PID=""
    fi
  fi

  # ── Option 3: localtunnel (via npx) ─────────────────────
  if command -v npx &> /dev/null; then
    echo "  🔍 Using localtunnel via npx…"
    npx -y localtunnel --port "$PORT" > /tmp/stationcommand-lt.log 2>&1 &
    TUNNEL_PID=$!

    for i in $(seq 1 40); do
      sleep 0.5
      TUNNEL_URL=$(grep -o 'https://[a-z0-9-]*\.loca\.lt' /tmp/stationcommand-lt.log 2>/dev/null | head -1)
      if [ -n "$TUNNEL_URL" ]; then break; fi
    done

    if [ -n "$TUNNEL_URL" ]; then
      echo "  ✅ localtunnel: $TUNNEL_URL"
      echo "  ⚠️  Note: localtunnel shows a reminder page on first visit."
      push_tunnel_url "$TUNNEL_URL"
      return 0
    else
      echo "  ⚠️  localtunnel didn't start in time."
      kill "$TUNNEL_PID" 2>/dev/null
      TUNNEL_PID=""
    fi
  fi

  # ── No tunnel available ──────────────────────────────────
  echo "  ❌ No tunnel tool found."
  echo ""
  echo "  Install one of these:"
  echo "    brew install ngrok          (macOS)"
  echo "    brew install cloudflared    (macOS)"
  echo "    npm install -g localtunnel  (any OS)"
  echo ""
  return 1
}

push_tunnel_url() {
  local URL="$1"
  curl -s -X POST "http://localhost:$PORT/api/tunnel" \
    -H "Content-Type: application/json" \
    -d "{\"url\": \"$URL\"}" > /dev/null 2>&1

  echo ""
  echo "  ╔═══════════════════════════════════════════════════╗"
  echo "  ║          📱  Share this with your audience        ║"
  echo "  ╠═══════════════════════════════════════════════════╣"
  echo "  ║                                                   ║"
  echo "  ║  $( printf '%-50s' "$URL/mobile.html" )║"
  echo "  ║                                                   ║"
  echo "  ╚═══════════════════════════════════════════════════╝"
  echo ""
  echo "  Display page: http://localhost:$PORT"
  echo "  Press Q on the display to show QR code"
  echo ""
}

start_tunnel

# ─── Keep alive ──────────────────────────────────────────────
echo "  Press Ctrl+C to stop everything."
echo ""
wait "$SERVER_PID"
